    ...
    Object.keys(learningModes).forEach(mode => {
        try {
            ...
            if ((window as any).errorManager && (window as any).errorManager.getErrorCount) {
                ...
                errorCount = state.progress.wordsToRepeatByMode[mode as import('../../core/types/trainer').ModeId]?.size || 0;
            }
...
export function updateCategoryStats(dom: DOMElements, state: TrainerState, learningModes: LearningModes): void {
    ...
    Object.keys(learningModes).forEach(modeId => {
        ...
        const progressData = state.progress.globalProgress[progressKey]?.[modeId as import('../../core/types/trainer').ModeId];
