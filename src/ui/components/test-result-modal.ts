...
declare global {
  interface Window {
    ...
    // state ist bereits in global.d.ts definiert
    rankingService?: {
...
export function showTestResultModal(testResult: TestResult, testConfig?: Record<string, unknown>) {
  ...
  document.getElementById('practice-incorrect-words')?.addEventListener('click', () => {
    ...
    if ((window as any).state?.training?.currentMode && (window as any).state?.progress) {
      const mode = (window as any).state.training.currentMode;
      if (!(window as any).state.progress.wordsToRepeatByMode[mode]) {
        (window as any).state.progress.wordsToRepeatByMode[mode] = new Set();
      }
      incorrectWordIds.forEach(id => {
        (window as any).state!.progress!.wordsToRepeatByMode[mode].add(id);
      });
    ...
    if ((window as any).setMode && (window as any).state?.training.currentMode) {
      (window as any).setMode((window as any).state.training.currentMode, true); // true = Wiederholungsmodus
    }
  });
